...
function ModalSetRake() {
  const { isSetRakeModalOpen, closeSetRakeModal } =
    ...
    const createTable = (child: any, open: boolean, parentId: number) => {
      return (
        <>
          <td colSpan={4} className="p-2 border border-gray-600">
            <UserTable
              parentId_={parentId}
              child={child}
  ...
  return (
    <Modal
      className="modal-set-rake"
      title="Set Rake to User"
      open={isSetRakeModalOpen}
      onCancel={closeSetRakeModal}
      footer={[
        <div key="close" className="flex gap-2 justify-center">
          <button
            className="px-4 py-1.5 rounded-md bg-brand-dialog-button"
            onClick={closeSetRakeModal}
          >
            Close
          </button>
        </div>,
