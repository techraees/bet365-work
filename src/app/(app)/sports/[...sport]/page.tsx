...
const Home = async ({ params }: any) => {
    ...
    if (sport[1] && sport[1] == "leagues") {
      ...
      return (
        <NatsSoccer
          ...
          leagueSelectedGames={leagueSelectedGames}
        />
      );
    }
  ...
  if (sport[0] === "baseball") {
    ...
    return <NatsBaseball odds={odds} sport={sport} getLeagues={getLeagues} />;
  }
  if (sport[0] === "cricket") {
    let getLeagues = await getPregameLeagues(sport[0]);
    let odds = await getPregameNames();
    return <NatsCricket odds={odds} sport={sport} getLeagues={getLeagues} />;
  }
  if (sport[0] === "darts") {
    let getLeagues = await getPregameLeagues(sport[0]);
    let odds = await getPregameNames();
    return <NatsDarts odds={odds} sport={sport} getLeagues={getLeagues} />;
  }
  if (sport[0] === "boxing") {
    let getLeagues = await getPregameLeagues(sport[0]);
    let odds = await getPregameNames();
    return <NatsBoxing odds={odds} sport={sport} getLeagues={getLeagues} />;
  }
  if (sport[0] === "basketball") {
    ...
    const odds = await Promise.all(promise);
    console.log({ odds });
    return <NatsBasketball odds={odds} sport={sport} getLeagues={getLeagues} />;
  }
  if (sport[0] === "esports") {
    let getLeagues = await getPregameLeagues(sport[0]);
    let promise = getLeagues?.map(async (league: string) => {
    ...
    return <NatsEsport odds={odds} sport={sport} getLeagues={getLeagues} />;
  }
  ...
  if (sport[0] === "hockey") {
    let getLeagues = await getPregameLeagues(sport[0]);
    let odds = await getPregameNames();
    return <NatsHockey odds={odds} sport={sport} getLeagues={getLeagues} />;
  }
  if (sport[0] === "handball") {
    let getLeagues = await getPregameLeagues(sport[0]);
    let odds = await getPregameNames();
    return <NatsHandball odds={odds} sport={sport} getLeagues={getLeagues} />;
  }
  if (sport[0] === "tennis") {
    let getLeagues = await getPregameLeagues(sport[0]);
    let promise = getLeagues?.map(async (league: string) => {
    ...
    const odds = await Promise.all(promise);
    console.log({ odds });
    return <NatsTennis odds={odds} sport={sport} getLeagues={getLeagues} />;
  }
  if (sport[0] === "volleyball") {
    ...
    const odds = await Promise.all(promise);
    console.log({ odds });
    return <NatsVolleyball odds={odds} sport={sport} getLeagues={getLeagues} />;
  }
  return (
