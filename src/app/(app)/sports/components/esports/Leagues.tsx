...
import { getEventNamesForLeague } from "@/api";

const LeagueWraps = ({ getLeagues }: any) => {
  console.log({ getLeagues });
  return (
    <div className="flex flex-col h-full w-full text-base">
      {getLeagues.map((item: any, index: number) => {
        return <Competitions name={item} key={index} />;
      })}
    </div>
  );
};
...
const Competitions = ({ name, odd }: any) => {
  ...
  const [gamesProps, setGameProps] = useState([]);
  ...
  const handleMainClick = async () => {
    toggleHeight();
    var data = await getEventNamesForLeague("esports", formattedName);
    setGameProps(data);
  };
  console.log({ odd });
            ...
            {Object.keys(details).map((item, index) => {
              return (
                <SubCompetitions
                  key={index}
                  item={item}
                  data={details[item as keyof typeof details]}
                  position={index}
                  name={name}
                />
              );
              ...
              return (
                <div
                  ...
                  onClick={() => {
                    ...
                    router.push(`sports/esports/leagues/${name}`);
                  }}
                >
