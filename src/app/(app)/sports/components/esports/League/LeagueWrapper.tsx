...
const LeagueWrapper = ({ sport, league, getLeagues }: any) => {
  const [active, setActive] = useState("Lines");
  const router = useRouter();
  const tabs = {
    tab: ["Lines", "Props", "Bet Boost", "Futures", "Table"],
  };
  console.log({ hi: sport, tt: league, tt2: getLeagues });
  const [odds, setOdds] = useState(null);

  useEffect(() => {
    async function fetchData() {
      try {
        const response = await getPregame(sport, league);
        setOdds(response);
      } catch (error) {
  ...
  return (
    <div className="text-base">
      <div className="min-h-[80px] w-full bg-[linear-gradient(rgba(12,22,20,.1),transparent_20px),radial-gradient(122%_370px_at_center_-220px,#009969_0,transparent_100%),linear-gradient(to_right_bottom,#0c1614,#084436)]">
        ...
        >
          <div className="flex items-center min-h-[45px] text-[12px]  px-[30px] py-0 mt-[5px]">
            ...
            <div className="pl-[5px]">{`Esport`}</div>
          </div>
        </button>
      ...
      <LeagueBody leagueSelectedGames={odds} active={active} />
    </div>
  );
};
