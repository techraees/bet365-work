...
import { esportMainMarkets } from './mappings/datastructure';
import { gameLines , firsthalf, firstQ} from './mappings/pregamemaps';
...
const MatchBody: React.FC<MarketGroupProps> = ({ data, active }) => {
    ...
    let oddData = {} as any;
    oddData = esportMainMarkets as any;
    if (active === "Main Markets" || active === "All") {
        esportMainMarkets.gameLines.rows = gameLines(data);
        esportMainMarkets['1stHalf'].rows = firsthalf(data);
        esportMainMarkets['1stQuarter'].rows = firstQ(data);
        oddData = esportMainMarkets as any;
    }
    console.log('detail market esport', oddData)
