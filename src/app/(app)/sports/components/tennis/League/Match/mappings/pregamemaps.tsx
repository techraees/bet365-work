            ...
            fmatchWrap.map((fmatchItem: any) => {
                ...
                if (fmatchItem.ismain === "True") {
                    fmatchItem?.odds.map((fod: any) => {
                        ...
                        arr.push({ title: title, value: fod.value, suspend: fod.stop === 'False' ? '0' : '1' })
                    })
                }
            })
            tosend.push(arr)
                ...
                fmatchItem?.odds.map((fod: any) => {
                    if (arr.length == 0) {
                        if (fod.name.startsWith('Home')) {
                            arr.push({ title: data?.player_1?.name, value: '', suspend: '0' })
                        } else if (fod.name.startsWith('Away')) {
                            arr.push({ title: data?.player_2?.name, value: '', suspend: '0' })
                        }
                    }
                    if (arr.length >= 3) {
                        return;
...
export const homeAwayTo = (data: any, main: any, team: any) => {
    ...
    if (team === 'home') {
        ...
        main.marketname = main.marketname.replace('Away', data?.player_2?.name)
    }
