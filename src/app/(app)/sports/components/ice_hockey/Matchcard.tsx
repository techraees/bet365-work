...
const Matchcard: FC<PostsProps> = ({ listOfData }) => {
  ...
  if (listOfData && listOfData.length > 0) {
    ...
    listOfData?.map((item: any) => {
      ...
      const home = item?.odds.filter((odd: any) => odd.id === "2")[0]
        ?.bookmakers[0]?.odds[0].value;
      const away = item?.odds.filter((odd: any) => odd.id === "2")[0]
        ?.bookmakers[0]?.odds[1].value;
        ...
        {data?.map((item: any, index: number) => {
          return (
            <SwiperSlide
              ...
              onClick={() => {
                router.push(
                  `sports/hockey/leagues/${item?.league}/${item?.id}`
                );
              }}
            >
