...
const UserBet = () => {
    ...
    useEffect(()=>{
        const coupons = async()=>{
            ...
            let cp = await getCoupons(token)
            let cp1= await cp.json()
            console.log({coupons: cp1 , token})
        }
        coupons()
    }, [])
        ...
        <div className=" w-[450px] bg-white text-sm fixed bottom-0 left-1/2 transform -translate-x-1/2 z-[1000]">
            <div className="flex flex-col">
                ...
                <div className="flex w-full">
                    <button className="h-[50px] flex-1 bg-[#333] text-[#58d7af] font-bold text-base">Set Stake</button>
                    <button className="h-[50px] flex-1 bg-[#58d7af] text-[#1f4d3e] font-bold text-base">Accept Change</button>
                </div>
            </div>
        </div>
    )
