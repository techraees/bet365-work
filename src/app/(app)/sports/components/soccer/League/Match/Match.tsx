...
import { getEventFromLeague } from "@/api";

interface Team {
  name: string;
  // add other team properties if necessary
}

interface Match {
  localteam: Team;
  visitorteam: Team;
  // include other match properties as needed
}
const Match = ({ gameid, league, getLeagues, leagueSelectedGames }: any) => {
  ...
  useEffect(() => {
    const fetchMatch = async () => {
      try {
        const match = await getEventFromLeague(league, gameid);
        setMatch(match); // Update the state with fetched data
      } catch (error) {
        console.error("Error fetching featured matches:", error);
      }
    };

    fetchMatch(); // Call the async function
      ...
      <div className="min-h-[80px] w-full bg-[linear-gradient(rgba(12,22,20,.1),transparent_20px),radial-gradient(122%_370px_at_center_-220px,#009969_0,transparent_100%),linear-gradient(to_right_bottom,#0c1614,#084436)]">
        ...
        >
          <div className="flex items-center min-h-[45px] text-[12px]  px-[30px] py-0 mt-[5px]">
            ...
            <div className="pl-[5px]">{`Soccer ${decodeURIComponent(
              league,
            ).replace(":", "")}`}</div>
          </div>
        </button>
