      ...
      if (item === "Popular") {
        ...
        uk.map((rest, index) => {
          if (leaguebycountry[rest] && leaguebycountry[rest].length > 0) {
            const toPush =
              leaguebycountry[rest]?.map(
                (listItem: string) => `${rest} ${listItem}`
              ) || [];
            arr.push(...toPush);
          }
        });
        ...
        const toPush =
          leaguebycountry[item]?.map(
            (listItem: string) => `${item} ${listItem}`
          ) || [];
        countrygroup[item] = [...toPush];
      } else if (item === "Internationals") {
        const toPush =
          leaguebycountry?.World?.map((listItem: string) => `${listItem}`) ||
          [];
        countrygroup[item] = [...toPush];
      } else if (item === "Europe") {
        let arr = [] as any;
        ...
        const toPush =
          leaguebycountry?.Ireland?.map(
            (listItem: string) => `Ireland ${listItem}`
          ) || [];
        countrygroup[item] = [...toPush];
        ...
        americas.map((rest, index) => {
          if (index === 0 || index === 1) {
            const toPush =
              leaguebycountry[rest]?.map((listItem: string) => `${listItem}`) ||
              [];
            arr.push(...toPush);
          } else if (
            leaguebycountry[rest] &&
            leaguebycountry[rest].length > 0
          ) {
            const toPush =
              leaguebycountry[rest].map(
                (listItem: string) => `${rest} ${listItem}`
            ...
            {Object.keys(countrygroup).map((item, index) => {
              return (
                <SubCompetitions
                  key={index}
                  item={item}
                  data={countrygroup[item as keyof typeof countrygroup]}
                  position={index}
                />
              );
            })}
  ...
  return (
    ...
    >
      ...
      >
        <div
          className={
            "text-[13px] leading-5  h-[50px] flex items-center font-[700]"
          }
        >
          {item}
        </div>
                    ...
                    if (Correction[dataitem as keyof typeof Correction]) {
                      console.log(
                        ...
                        `sports/soccer/leagues/${
                          Correction[dataitem as keyof typeof Correction]
                        }`
                      );
                      router.push(
                        `sports/soccer/leagues/${
