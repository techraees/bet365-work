
import {
    BaseballIcon,
    BasketBallIcon,
    CricketIcon,
    HandballIcon,
    SoccerIcon,
    TennisIcon,
    EsportsIcon,
    AmericanFootballIcon,
    AustralianRulesIcon,
    BowlsIcon,
    BoxingIcon,
    CasinoIcon,
    CyclingIcon,
    DartsIcon,
    FantasySportsIcon,
    Formula1Icon,
    FutsalIcon,
    GaelicSportsIcon,
    GolfIcon,
    GreyhoundsIcon,
    HorseRacingIcon,
    IceHockeyIcon,
    LottoIcon,
    MMAIcon,
    PoliticsIcon,
    RugbyLeagueIcon,
    RugbyUnionIcon,
    SnookerIcon,
    SpeedwayIcon,
    TableTennisIcon,
    VolleyballIcon,
    TrottingIcon,
    VirtualSportsIcon
} from "@/components/ui/icons/sports";

export const navItems = [
    {
        label: "Soccer",
        link: "/sports/soccer",
        icon: SoccerIcon,
    },
    {
        label: "Baseball",
        link: "/sports/baseball",
        icon: BaseballIcon,
    },
    {
        label: "Basketball",
        link: "/sports/basketball",
        icon: BasketBallIcon,
    },
    {
        label: "Boxing",
        link: "/sports/boxing",
        icon: BoxingIcon,
    },
    {
        label: "Cricket",
        link: "/sports/cricket",
        icon: CricketIcon,
    },
    {
        label: "Darts",
        link: "/sports/darts",
        icon: DartsIcon,
    },
    {
        label: "Esports",
        link: "/sports/esports",
        icon: EsportsIcon,
    },
    {
        label: "Ice Hockey",
        link: "/sports/ice-hockey",
        icon: IceHockeyIcon,
    },
    {
        label: "Handball",
        link: "/sports/handball",
        icon: HandballIcon,
    },
    {
        label: "Volleyball",
        link: "/sports/volleyball",
        icon: VolleyballIcon,
    },
    {
        label: "Table Tennis",
        link: "/sports/table-tennis",
        icon: TableTennisIcon,
    },
    {
        label: "Tennis",
        link: "/sports/tennis",
        icon: TennisIcon,
    },
    {
        label: "American Football",
        link: "/sports/american-football",
        icon: AmericanFootballIcon,
    },
    {
        label: "Australian Rules",
        link: "/sports/australian-rules",
        icon: AustralianRulesIcon,
    },

    {
        label: "Bowls",
        link: "/sports/bowls",
        icon: BowlsIcon,
    },

    {
        label: "Casino",
        link: "/sports/casino",
        icon: CasinoIcon,
    },

    {
        label: "Cycling",
        link: "/sports/cycling",
        icon: CyclingIcon,
    },


    {
        label: "Fantasy Sports",
        link: "/sports/fantasy-sports",
        icon: FantasySportsIcon,
    },
    {
        label: "Formula 1",
        link: "/sports/formula-1",
        icon: Formula1Icon,
    },
    {
        label: "Futsal",
        link: "/sports/futsal",
        icon: FutsalIcon,
    },
    {
        label: "Gaelic Sports",
        link: "/sports/gaelic-sports",
        icon: GaelicSportsIcon,
    },
    {
        label: "Golf",
        link: "/sports/golf",
        icon: GolfIcon,
    },
    {
        label: "Greyhounds",
        link: "/sports/greyhounds",
        icon: GreyhoundsIcon,
    },
    {
        label: "Horse Racing",
        link: "/sports/horse-racing",
        icon: HorseRacingIcon,
    },

    {
        label: "Lotto",
        link: "/sports/lotto",
        icon: LottoIcon,
    },
    {
        label: "MMA",
        link: "/sports/mma",
        icon: MMAIcon,
    },
    {
        label: "Motor Sports",
        link: "/sports/motor-sports",
        icon: "",
    },
    {
        label: "Politics",
        link: "/sports/politics",
        icon: PoliticsIcon,
    },
    {
        label: "Rugby League",
        link: "/sports/rugby-league",
        icon: RugbyLeagueIcon,
    },
    {
        label: "Rugby Union",
        link: "/sports/rugby-union",
        icon: RugbyUnionIcon,
    },
    {
        label: "Snooker",
        link: "/sports/snooker",
        icon: SnookerIcon,
    },
    {
        label: "Soccer",
        link: "/sports/soccer",
        icon: SoccerIcon,
    },
    {
        label: "Specials",
        link: "/sports/specials",
        icon: "",
    },
    {
        label: "Speedway",
        link: "/sports/speedway",
        icon: SpeedwayIcon,
    },

    {
        label: "Trotting",
        link: "/sports/trotting",
        icon: TrottingIcon,
    },
    {
        label: "Virtual Sports",
        link: "/sports/virtual-sports",
        icon: VirtualSportsIcon,
    },
    {
        label: "Volleyball",
        link: "/sports/volleyball",
        icon: VolleyballIcon,
    },
    {
        label: "Winter Sports",
        link: "/sports/winter-sports",
        icon: "",
    },
    {
        label: "More Sports",
        link: "", // Replace this with the actual link if available
        icon: "", // Replace this with the actual icon URL if available
    },
    {
        label: "Odds On Coupon",
        link: "", // Replace this with the actual link if available
        icon: "", // Replace this with the actual icon URL if available
    },
    {
        label: "Top Events",
        link: "", // Replace this with the actual link if available
        icon: "", // Replace this with the actual icon URL if available
    }
]



export const navDropdowns = [
    "Motor Sports",
    "Specials",
    "Winter Sports",
    "More Sports",
]