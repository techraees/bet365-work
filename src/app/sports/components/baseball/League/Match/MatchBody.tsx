...
import { baseballMainMarkets, baseballBetBuilder, baseballInningsProps, baseballMainProps, baseballPlayerProps, baseballTeamProps } from './mappings/datastructure';
import { fiveInnings, gameLines , } from './mappings/pregamemaps';
...
const MatchBody: React.FC<MarketGroupProps> = ({ data, active }) => {
    ...
    let oddData = {} as any;
    oddData = baseballMainMarkets as any;
    if (active === "Main Markets" || active === "All") {
        baseballMainMarkets.gameLines.rows = gameLines(data);
        baseballMainMarkets["5Innings"].rows = fiveInnings(data);
        oddData = baseballMainMarkets as any;
