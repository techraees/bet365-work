...
const MarketCell: React.FC<MarketCellProps> = ({
  ...
  const handleClick = (message: any) => {
    console.log("Div clicked!", message);
    // Add your logic here
    addSelection(message);
  };
  return (
    <div
      onClick={() => handleClick(target_odd)}
      className={cn(
        "col-span-1 flex items-center h-[100%] text-[13px] cursor-pointer relative",
        textLeftAlign ? "" : "justify-center",
        suspend !== "0" ? "text-[#a7a7a7]" : "",
        disablehover || (!value && !name)
          ? "cursor-auto"
          : "hover:bg-[hsla(0,0%,100%,.15)] ",
    ...
    >
      ...
      {borderRight && (
        <div className="absolute right-0 bg-[#ffffff1a] flex items-center h-[calc(100%_-_20px)] w-[1px]" />
      )}
      {separation && (
        <div className="absolute right-0 bg-[#ffffff1a] flex items-center h-[100%] w-[1px]" />
      )}
    </div>
  );
};
...
export const MarketCellSplit: React.FC<MarketCellProps> = ({
  name,
  value,
  disablehover,
  borderRight,
