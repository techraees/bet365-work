...
import { categoriesMapping, hockeyCategoriesMapping } from "@/lib/sportsMapping";

import { cn } from "@/lib/utils";
interface GoalLineInPlayProps {
    data: any;
}

const GoalLineInPlay: React.FC<GoalLineInPlayProps> = ({ data }) => {
    let objs: any[] = []
    let suspend: string = "01";
    Object.keys(data.odds).map(item => {
        if (data.odds[item].name === hockeyCategoriesMapping["periodlines"]) {
