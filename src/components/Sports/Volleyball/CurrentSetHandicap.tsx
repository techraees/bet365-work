...
interface CurrentSetHandicap {
    data: any;
}
const CurrentSetHandicap: React.FC<CurrentSetHandicap> = ({data}) => {
  let obj2:any[] = []
  let objs: any[] = []
  if(!data){
      return null
  ...
  Object.keys(data?.odds).map((item:any) => {
      obj2.push(data?.odds[item].name)
    if (data.odds[item].name === "Home/Away (1st Set)") {
...
export default CurrentSetHandicap;
