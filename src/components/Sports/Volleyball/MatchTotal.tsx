...
interface MatchHandicapSets {
  data: any;
}
const MatchHandicapSets: React.FC<MatchHandicapSets> = ({ data }) => {
  let obj2: any[] = [];
  let objs: any[] = [];
  if (!data) {
    return null;
  ...
  Object.keys(data?.odds).map((item: any) => {
    if (data.odds[item].name === "Total Points Odd/Even (3rd Set)") {
      ...
      Object.keys(participants).map((participat) => {
        if (
          participants[participat]?.name === "Home" &&
          participants[participat]?.suspend === "0"
        ) {
          home.push(participants[participat].value_eu);
        } else if (
          participants[participat]?.name === "Away" &&
          participants[participat]?.suspend === "0"
        ) {
  ...
  console.log(
    obj2,
    "I am Object 2 here to show you!",
    Object.keys(data?.odds).length
  );
  return (
...
export default MatchHandicapSets;
