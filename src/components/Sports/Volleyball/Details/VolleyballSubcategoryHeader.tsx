...
const VolleyballSubcategoryHeader: React.FC<GropedEventsInterface> = ({ name, events, sport, subcategory }) => {
    return (
        <div className="flex flex-col text-xs font-bold text-white mb-[5px]">
            ...
            {events.map((event: any, index: number) => (
                <VolleyballEvent key={index} data={event} sport={sport} subcategory={subcategory} />
            ))}
        </div>
    )
}
