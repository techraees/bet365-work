...
export const map5Winner = (data: any) => {
    ...
    let rows = [] as any;
    let row = [] as any;
    if (data?.odds?.[1182195]?.participants) {
        const items: any = Object.entries(data?.odds?.[1182195]?.participants);
        items.map((itm: any) => {
            let title = '';
            const item = itm[1];
            title = item?.name;
        ...
        if (items.length > 0) {
            items.map((item: any, index: number) => {
                ...
                if (killHandicap.length < 3) {
                    killHandicap.push({ title: ``, value: item[1].value_eu, suspend });
                }
            })
        }
    }
        ...
        if (items.length > 0) {
            items.map((item: any, index: number) => {
                ...
                if (firstBlood.length < 3 ) {
                    firstBlood.push({ title: ``, value: item[1].value_eu, suspend });
                }
            })
        }
    }
        ...
        if (items.length > 0) {
            items.map((item: any, index: number) => {
                ...
                if (firstTower.length < 3) {
                    firstTower.push({ title: ``, value: item[1].value_eu, suspend });
                }
            })
        }
    }
