...
export const map1Kills = (data: any) => {
  ...
  const header = ["", data?.team_info?.home?.name, data?.team_info?.away?.name];
  const killHandicap = [
    { title: "Kill Handicap", value: null, suspend: 0 },
  ] as any;
        ...
        if (killHandicap.length < 3) {
          killHandicap.push({
            ...
            event_name:
              ...
              data?.team_info?.away?.name,
            odd_id: 101001001,
            odd_name: data?.odds?.[101001001]?.name,
            participant_id: item[1]?.id,
            participant_name: item[1]?.name,
            participant_handicap: item[1]?.handicap,
            participant_header: item[1]?.header,
        ...
        if (firstBlood.length < 3) {
          firstBlood.push({
            ...
            event_name:
              ...
              data?.team_info?.away?.name,
            odd_id: 1182595,
            odd_name: data?.odds?.[1182595]?.name,
            participant_id: item[1]?.id,
            participant_name: item[1]?.name,
            participant_handicap: item[1]?.handicap,
            participant_header: item[1]?.header,
  ...
  // console.log('Sending New Data', data)
  let rows = [] as any;
  if (killHandicap.length > 1) rows.push(killHandicap);
  if (firstBlood.length > 1) rows.push(firstBlood);
      ...
      if (row.length < 3 && title === "Home" && item.suspend == "0") {
        row.push({
          ...
          event_name:
            data?.team_info?.home?.name + " vs " + data?.team_info?.away?.name,
          odd_id: 1182295,
          odd_name: data?.odds?.[1182295]?.name,
          participant_id: item?.id,
          participant_name: item?.name,
          participant_handicap: item?.handicap,
          participant_header: item?.header,
...
export const map6Towers = (data: any) => {
        ...
        if (firstTower.length < 3) {
          firstTower.push({
            ...
            event_name:
              ...
              data?.team_info?.away?.name,
            odd_id: 9591565,
            odd_name: data?.odds?.[9591565]?.name,
            participant_id: item[1]?.id,
            participant_name: item[1]?.name,
            participant_handicap: item[1]?.handicap,
            participant_header: item[1]?.header,
  ...
  // console.log('Sending New Data', data)
  let rows = [] as any;
  if (firstTower.length > 1) rows.push(firstTower);
  return { header, rows: rows };
};
//map6FirstTyrant
        ...
        if (firstBlood.length < 3) {
          firstBlood.push({
            ...
            event_name:
              ...
              data?.team_info?.away?.name,
            odd_id: 1182695,
            odd_name: data?.odds?.[1182695]?.name,
            participant_id: item[1]?.id,
            participant_name: item[1]?.name,
            participant_handicap: item[1]?.handicap,
            participant_header: item[1]?.header,
  ...
  if (data?.odds?.[9591584]?.participants) {
    ...
    if (items.length > 0) {
      items.map((item: any, index: number) => {
        ...
        title = item[1]?.name;
        if (row.length == 0) {
          row.push({ title: item[1].handicap, value: "", suspend: suspend });
        }
        if (row.length < 3) {
          row.push({
            title: ``,
            ...
            event_name:
              ...
              data?.team_info?.away?.name,
            odd_id: 9591584,
            odd_name: data?.odds?.[9591584]?.name,
            participant_id: item[1]?.id,
            participant_name: item[1]?.name,
            participant_handicap: item[1]?.handicap,
            participant_header: item[1]?.header,
