...
const Dialog: React.FC<DialogProps> = ({ isOpen, onClose, grouped, currentdataId, currentGroupName }) => {
    ...
    return (
        <div className='z-[1000] absolute overflow-hidden top-[55px] left-[40px] w-[500px] flex flex-col bg-[linear-gradient(155deg,#1e2a27,#133d30)]'>
            
            <div className='absolute  z-10 top-[-200px] left-[calc(50%_-_150px)] w-[300px] h-[300px] blur-[50px] rounded-[100%] bg-[#28ffbb40]'>
            ...
            {grouped?.map((data: { name: string }, index: number) => {
                return (
                    <div key={index} className={cn('z-20',index!==0? 'border-t-[#ffffff1a] border-t border-solid':'')}>
                        <div
                            className={cn("w-full flex justify-center cursor-pointer pl-[30px] pr-[15px] text-[#00ffb6] hover:text-[white]")}
                            onClick={() => {
                                setShow({
                                    ...show,
                        ...
                        >
                            <div className={cn('text-[13px] h-[50px] flex items-center font-[700]')}>
                                {data.name}
                            </div>

                        </div>

                        <div className={cn(' flex flex-col overflow-hidden transition-[max-height] duration-300 ease',
                            show[data.name]? 'max-h-[5000px]' : 'max-h-[0px]')}>
                            {
                            data?.events.map((event, index) => {
                                return (
                                    <div key={event?.info?.id}
                                        className={cn('h-[100%] text-[13px] font-[700]  overflow-hidden hover:bg-[#ffffff1a]', event?.info?.id === currentdataId? 'bg-[#ffffff1a]': '')}>
